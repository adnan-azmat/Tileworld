...
public class TeamAgent3 extends TWAgent{
    ...
    // private void cPlanSearchPointInitial(int assignedZone) {
    ...
    //             cPlanSearchPoint.add(new int[] {mapsizeX / 3 - 4, 3});
    //             cPlanSearchPoint.add(new int[] {mapsizeX / 3 - 4, mapsizeY - 4});
    //             cPlanSearchPoint.add(new int[] {3, mapsizeY - 4});
    //             break;
    //         case 1:
    //             cPlanSearchPoint.clear();
    //             cPlanSearchPoint.add(new int[] {mapsizeX / 3 + 3, 3});
    //             cPlanSearchPoint.add(new int[] {(mapsizeX * 2) / 3 - 4, 3});
    //             cPlanSearchPoint.add(new int[] {(mapsizeX * 2) / 3 - 4, mapsizeY - 4});
    //             cPlanSearchPoint.add(new int[] {mapsizeX / 3 + 3, mapsizeY - 4});
    //             break;
    //         case 2:
    //             cPlanSearchPoint.clear();
    //             cPlanSearchPoint.add(new int[] {(mapsizeX * 2) / 3 + 3, 3});
    //             cPlanSearchPoint.add(new int[] {mapsizeX - 4, 3});
    //             cPlanSearchPoint.add(new int[] {mapsizeX - 4, mapsizeY - 4});
    //             cPlanSearchPoint.add(new int[] {(mapsizeX * 2) / 3 + 3, mapsizeY - 4});
    //             break;
    //     }
    
    //     addTempMessage("cPlanSearchPointInitial byZone");
    ...
    private void get_scheme(){
        ...
        double agent3Score = memory_tile_score(this.getX(), this.getY(), this.carriedTiles.size());
        double agent4Score = memory_tile_score(this.otherAgentPosition[0], this.otherAgentPosition[1], otherCarriedTiles);

        System.out.println("Getting get_scheme!!!, toFuel="+toFuelScore+"  toAgent3="+agent3Score+"  toAgent4="+agent4Score);
        ...
        if ( "A".equals(AgentParameter.scheme) ){  // Plan A
            if (agent3Score < AgentParameter.aPlanSearchScore ||
                ...
                if (agent4Score < AgentParameter.aPlanSearchScore ||
        ...
        if ("B".equals(AgentParameter.scheme)){  // Plan B
            agent3Score = bPlanAgent1PickScore();
            if (agent3Score < AgentParameter.bPlanPickScore){
        ...
        if ("C".equals(AgentParameter.scheme)){  //Plan C
            // cPlanSearchPointInitial(this.assignedZone);
    ...
    private int[] getPossiblePick(){
        ...
        if ("agent3".equals(this.name)) allowDis = AgentParameter.agent1pickDis;
    ...
    protected TWThought think() {
        ...
        if (this.rethinking==0 && "agent3".equals(this.name)) this.addTempMessage("MyEstimateLifeTime "+getELT());
        ...
        if ("PickingTile".equals(this.agentState1)){
            ...
            if (this.agentState2 == 1){
                if ("agent3".equals(this.name) && bPlanAgent1PickScore() < AgentParameter.bPlanPickScore){
                    get_scheme();
                    rethinking=1;
                    return think();
                }
